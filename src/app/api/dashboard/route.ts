...
export async function GET() {
  try {
    // 1. Fetch vacancies and interviews
    ...
    // 2. Fetch pipeline details for all active vacancies in parallel
    const pipelines = await Promise.all(
      activeVacancies.map(async (v) => {
        try {
          ...
          return { vacancyId: String(v.id), stages: [], cards: [], rejectionSummary: { total: 0 } };
        }
      })
    );

    // 3. Aggregate KPI stats
    const totalApplicants = pipelines.reduce((sum, p) => sum + p.cards.length, 0);

    // Find hired candidates (cards in positive final stages)
    ...
    // 4. Group candidates by stage name (funnel)
    ...
    // 5. Group candidates by stage per vacancy
    ...
    const upcomingInterviews = interviewsPage.items
      .map((int) => {
        ...
        let dayLabel = date.toLocaleDateString("es-EC", { weekday: "short", day: "numeric", month: "short" });
        ...
        const timeLabel = date.toLocaleTimeString("es-EC", { hour: "2-digit", minute: "2-digit", hour12: false });
      ...
      .slice(0, 5); // top 5 interviews

    // 8. Map top candidates (match score >= 75%)
    const topCandidates = pipelines
      .flatMap((p) => {
        ...
        return p.cards.map((c) => ({
          ...
          daysAgo: 2, // mock value for display
        }));
      })
