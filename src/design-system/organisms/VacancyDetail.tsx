...
function TagPill({ label }: { label: string }) {
  return (
    <span className="px-2.5 py-1 bg-surface-2 text-ink text-[12px] rounded-full">
      {label}
    </span>
  );
}

/** Hint banner shown above the salary field. */
export interface VacancyApplyHint {
...
export interface VacancyDetailProps {
  ...
  /** Target of the "Vacantes" back link. */
  backHref: string;
  /** Label for the salary field (e.g. "Aspiración salarial"). */
  salaryLabel: string;
  /** Optional informational banner above the salary input. */
  applyHint?: VacancyApplyHint;
  /**
   * Performs the application with the parsed salary. Resolving marks the card as
   * applied. It may instead navigate away (e.g. redirect to login) and never
   * resolve — in that case the submit button stays in its loading state until
   * navigation completes, which is the desired UX.
   */
  onApply: (salary: number) => Promise<void>;
  /** Target of the "Ver mi postulación" button in the applied panel. */
  appliedHref: string;
  /** Extra content rendered inside the applied panel, below the title. */
  appliedExtra?: React.ReactNode;
}

/**
 * Candidate-facing vacancy detail. Pure presentation: all behaviour (how to
 * apply, where links point, contextual copy) is injected via props, so the same
 * layout serves both the public job board and the authenticated portal.
 */
export function VacancyDetail({
    ...
    try {
      ...
      setApplyError(
        err instanceof Error && err.message
          ? err.message
          : 'No fue posible enviar tu postulación. Por favor, intenta de nuevo.',
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const HintIcon = applyHint?.icon;

  return (
    <div className="flex flex-col gap-5">
      <Link
        href={backHref}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-surface border border-border text-[13px] text-ink-muted hover:text-ink hover:bg-surface-2 transition-colors w-fit"
      >
        <ArrowLeft size={15} />
        Vacantes
      </Link>
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start">
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="bg-card rounded-xl border border-border p-5 sm:p-7">
            ...
            {/* Description */}
            <div className="mb-6">
              <h3 className="text-[12px] text-ink-subtle uppercase font-semibold tracking-wide mb-3">
              ...
              <>
                ...
                {applyHint && HintIcon && (
                  <div className="bg-primary/8 rounded-lg p-3 flex items-start gap-3 mb-5">
                    <HintIcon size={18} className="text-primary shrink-0 mt-0.5" />
                    <p className="text-[13px] text-ink-muted">{applyHint.text}</p>
                  </div>
                )}

                <div className="mb-6">
                  <label className="block text-[13px] font-semibold text-ink mb-2 uppercase tracking-wide">
                    {salaryLabel} <span className="text-primary">*</span>
                  </label>
